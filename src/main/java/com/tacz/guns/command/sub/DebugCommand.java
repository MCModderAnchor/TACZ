...
public class DebugCommand {
    ...
    private static int setValue(CommandContext<CommandSourceStack> context) {
        ...
        if (context.getSource().getEntity() instanceof ServerPlayer serverPlayer) {
            if (DEBUG) {
                serverPlayer.sendMessage(new TextComponent("TacZ Debug Mode is Turn On"), Util.NIL_UUID);
            } else {
                serverPlayer.sendMessage(new TextComponent("TacZ Debug Mode is Turn Off"), Util.NIL_UUID);
            }
        }
