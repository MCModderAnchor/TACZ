...
public class ControllableInner {
    ...
    public void onAvailableActions(GatherActionsEvent event) {
        ...
        if (IGun.mainhandHoldGun(player)) {
            ...
            // 左侧显示主要按键：瞄准、开火，换弹
            actions.put(AIM, new Action(new TranslatableComponent("key.tacz.aim.desc"), Action.Side.LEFT));
            actions.put(SHOOT, new Action(new TranslatableComponent("key.tacz.shoot.desc"), Action.Side.LEFT));
            actions.put(RELOAD, new Action(new TranslatableComponent("key.tacz.reload.desc"), Action.Side.LEFT));

            // 右侧显示次要按键：近战，切换开火模式
            actions.put(MELEE, new Action(new TranslatableComponent("key.tacz.melee.desc"), Action.Side.RIGHT));
            actions.put(FIRE_SELECT, new Action(new TranslatableComponent("key.tacz.fire_select.desc"), Action.Side.RIGHT));
        }
    }
