    ...
    public static void onRenderOverlay(RenderGameOverlayEvent.PreLayer event) {
        if (event.getOverlay() == ForgeIngameGui.CROSSHAIR_ELEMENT) {
            ...
            TimelessAPI.getClientGunIndex(gunId).ifPresent(gunIndex -> {
                // 瞄准快要完成时，取消准心渲染
                if (!gunIndex.isShowCrosshair() && playerGunOperator.getClientAimingProgress(event.getPartialTicks()) > 0.9) {
                    return;
                }

                GunAnimationStateMachine animationStateMachine = gunIndex.getAnimationStateMachine();
                if (!animationStateMachine.shouldHideCrossHair()) {
                    renderCrosshair(event.getMatrixStack(), event.getWindow(), gunIndex.getCrosshairTextureLocation());
                }
            });
        }
    }
